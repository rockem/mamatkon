...
$includepath = "\"include ../include ../include/mamatkon/ui $boost_dir\"";
$config='"console qt debug"';
$libs='win32:LIBS+="../vslib/python26_d.lib vslib/gtestd.lib vslib/UnitTest++.vsnet2005.lib"';
$os = ($^O =~ /MSWin32/) ? 'win32-msvc2003' : 'unix';	
$files='src/ ../src/db/ ../src/control/ ../src/ui/ ../resource/ ../include/mamatkon ../src/command/';
$excluded_files='../src/main.cpp';
$qt='sql';
...
`qmake -project $files -nopwd -r $libs QMAKE_EXTRA_TARGETS+=$extra_targets QT+=$qt CONFIG+=$config SOURCES-=$excluded_files INCLUDEPATH+=$includepath -o $app`;
`qmake $app -spec $os`;
