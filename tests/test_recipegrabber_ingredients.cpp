...
protected:
		...
		currentRecipeId(3)
	{
		...
		eventManager.raiseEvent(shared_ptr<RecipeEvent> (new RecipeEvent(
				Event::SelectedRecipeChanged, currentRecipeId)));
	...
	{
		std::cout<<"id:"<<currentRecipeId<<" ==>"<<std::endl;
		for(unsigned int i = 0; i < ingredients.size(); ++i) {
			std::wcout<<ingredients[i]<<std::endl;
		}
